...
We will do an example of a bootstrapping for RNA-seq data collected for a differential expression analysis.2
...
Aca hizo la simulacion mezclando los controles con los tratados primero y despues agrupandolos correctamente. Asi logra que se noten las diferencias y demstras el poder del test de mostrar lo que realmente es diferente vs lo que se deduce solamente del ruido intrinseco de los datos.
...
Cuando hace el .3 es porque hace 1/3 que es como si fuese la media de la distribucion de poisson - Todo esto es arbitrario, y dependiendo del numero que se use el resultado va a ser diferente, por eso es importante poner que es lo que se hace e incluir el markdown en el paper.

el primer comando cambia a matriz porque sino algunos de los softwares no lo reconocen, y ademas saco la primera fila porque tiene los nombres de los genes
...
We start by replacing the zeroes by 0.25. Despues va a calcular pij=count/library size for each gene in each samples
despues calcula la media de pi para cada uno de los genes
genera la version "noisy" de pi con el promedio de pi y despues con los counts y al final genera poisson para las noisy counts (Esta todo aca abajo en los comandos)
...
While it might make sense to transform the means and SDs of the proportions into the parameters of a distribution like the Beta, which generates data between 0 and 1, this method did not work well for these data, as it created distributions for some genes that essentially had only 1 proportion. 

Instead, we add Normal noise (pero muy poquito!), scaled by the SD, to the mean.  After scaling up by the library size to obtain a mean count, we replaced all the means that are less than 0.25 by 0.25.
...
If we want to generate data under the non-Null distribution, we proceed in exactly the same way except the we compute the mean and SD of each gene for each condition. Solo cambiamos esa parte del proceso -
